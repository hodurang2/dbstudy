...
SELECT HIRE_DATE,
       CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) AS NEWHIRE_DATE 
  FROM EMPLOYEES;
...
SELECT HIRE_DATE,
       CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) AS NEWHIRE_DATE ,
       CASE
        WHEN CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) >= 240 THEN '퇴직금정산대상'
        ELSE ''
       END AS FIRST_NAME
  FROM EMPLOYEES;
...
-- 17. EMPLOYEES 테이블에서 DEPARTMENT_ID와 JOB_ID가 모두 같은 사원들을 그룹화하여 각 그룹의 사원수를 조회하시오. 
-- 조건 1) DEPARTMENT_ID가 NULL인 사원은 제외하시오.
SELECT DEPARTMENT_ID,
       JOB_ID,
       COUNT(*) AS 사원수
  FROM EMPLOYEES
