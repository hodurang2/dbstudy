/*
    ...
    3. 조인 종류
        1) 내부 조인 : 조회 대상이 2개 테이블에 모두 존재하는 데이터만 조회하는 방식 (중복된COLUMN은 반드시 테이블 명시)
            1-1)형식
                SELECT 테이블1.COL1,2,... 테이블2.COL1,2,...
                  FROM 부모테이블(1) INNER JOIN 자식테이블(M) -> 일반적으로 데이터 적은쪽이 앞 많은쪽을 뒤로둬서 성능도 생각한다.
                    ON 조인_조건
        2) 외부 조인 : 조회 대상이 2개 테이블에 모두 존재하지 않더라도 조회하는 방식
...
/*
    ...
    1. 드라이브 테이블
        ...
        3) 일반적으로 데이터의 갯수가 적다.
        4) PK를 조인 조건으로 사용하기 때문에 인덱스(INDEX) 사용이 가능하다. -> 빠르다 -> 성능향상
    2. 드리븐 테이블
        1) 1:M 관계에서 M에 해당하는 테이블
        2) 일반적으로 데이터의 갯수가 많다.
        3) FK를 조인 조건을 사용하기 때문에 인덱스(INDEX) 사용이 불가능하다 -> 느리다 -> 성능이 좋지않음
...
SELECT EMPLOYEE_ID,
       ...
       D.DEPARTMENT_ID,  -- 2개의 네이블에 모두있는 COLUMN은 반드시 테이블(오너)을 명시해야 한다.
       DEPARTMENT_NAME
  FROM DEPARTMENTS D RIGHT OUTER JOIN EMPLOYEES E  -- 오른쪽 테이블(EMPLOYEES)의 모든 데이터를 조회하시오.
...
--FROM DEPARTMENTS D LEFT OUTER JOIN EMPLOYEES E  -- 왼쪽 테이블(DEPARTMENTS)의 모든 데이터를 조회하시오.
  FROM EMPLOYEES E RIGHT OUTER JOIN DEPARTMENTS D -- 오른쪽 테이블(DEPARTMENTS)의 모든 데이터를 조회하시오.
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;
