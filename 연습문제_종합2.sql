...
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME, CUST_ADDR, CUST_TEL) VALUES(CUST_SEQ.NEXTVAL, '박지성', '영국', '010-000-000');
...
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES (ORDER_SEQ.NEXTVAL, 1000, 3, 2, '20/07/02');
...
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES (ORDER_SEQ.NEXTVAL, 1000, 2, 5, '20/07/06');
...
SELECT O.ORDER_ID,
 ...
 WHERE TO_DATE(O.ORDER_DT) BETWEEN TO_DATE('20/07/05') AND TO_DATE('20/07/09')
 ORDER BY O.ORDER_ID;
...
SELECT *
  FROM(SELECT RANK() OVER(ORDER BY ORDER_DT DESC) AS 주문일자, CUST_NAME, BOOK_NAME, ORDER_DT
         FROM BOOK_T B INNER JOIN ORDER_T O
           ON B.BOOK_ID = O.BOOK_ID INNER JOIN CUSTOMER_T C 
           ON O.CUST_ID = C.CUST_ID)
 WHERE 주문일자 = 1;
...
-- 김연아  2
-- 서브쿼리
SELECT CUST_NAME AS 고객명, (SELECT COUNT(*)
                               FROM ORDER_T
                              WHERE CUST_ID = 1001) AS 구매한도서수
  FROM CUSTOMER_T
 WHERE CUST_NAME = '김연아';
