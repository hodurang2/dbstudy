...
SELECT USER_NO,
       NVL(CONCAT(USER_MOBILE1, USER_MOBILE2), 'None')     
  FROM USER_T;
...
SELECT U.USER_NO AS 번호,
 ...
 WHERE USER_ID IN ('NHS', 'YJS', 'SDY', 'KKJ', 'LKK');

-- 강사님
SELECT USER_NO AS 번호,
       USER_ID AS 아이디
  FROM USER_T
 WHERE USER_NO NOT IN (SELECT DISTINCT USER_NO
                         FROM BUY_T);
...
SELECT PROD_NAME AS  제품명, 
       CONCAT(COUNT(B.PROD_CODE), '개') AS 판매횟수
...
SELECT USER_NAME AS 고객명,
       PROD_NAME AS 제품명,
       BUY_AMOUNT * PROD_PRICE AS 구매액
  FROM PRODUCT_T P INNER JOIN BUY_T B
    ON P.PROD_CODE = B.PROD_CODE INNER JOIN USER_T U
    ON B.USER_NO = U.USER_NO
 WHERE B.PROD_CODE IN (SELECT PROD_CODE
                         FROM PRODUCT_T
...
SELECT USER_ID,
  ...
  FROM PRODUCT_T P INNER JOIN BUY_T B
    ON P.PROD_CODE = B.PROD_CODE INNER JOIN USER_T U
    ON B.USER_NO = U.USER_NO
 GROUP BY U.USER_NO, U.USER_ID, U.USER_NAME;
...
SELECT USER_NAME AS 고객명,
  ...
  FROM BUY_T B INNER JOIN USER_T U
    ON B.USER_NO = U.USER_NO   
 GROUP BY U.USER_NO, USER_NAME
HAVING COUNT(U.USER_NO) >= 2;
...
UPDATE USER_T
   SET USER_MOBILE1 = '011'
 WHERE USER_MOBILE1 = '010';
...
SELECT *
  FROM BUY_T;
-- 결과 : BUY_T의 PROD_CDDE가 1이 사라지고 NULL값이 되었다.
