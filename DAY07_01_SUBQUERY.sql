/*
    ...
    2. 서브쿼리를 먼저 실행해서 그결과를 메인쿼리에 전달한다.
    3. 종류
        1) SELECT 절 : 스칼라 서브쿼리
        2)   FROM 절 : 인라인 뷰 (INLINE VIEW)
        3)  WHERE 절 : 단일 행 서브쿼리 (결과가 1개) -> 무조건 1개
                       다중 행 서브쿼리 (결과가 N개) -> 0개도 가능
*/
/*
...
SELECT *
 ...
 WHERE JOB_ID = (SELECT JOB_ID
                   FROM EMPLOYEES
                  WHERE EMPLOYEE_ID = 101); -- PK로 SELECT했기때문에 단일 행인거같음
...
/*
    FROM 절의 서브쿼리 -> 나중에 실무에서 젤중요한거ㅍ같읍
*/
-- 게시판 CRUD
...
-- 3. 21 ~ 30 째로 입사한 사원 조회하기
SELECT RN, EMPLOYEE_ID, HIRE_DATE
...
*/
-- 부서번호가 50인 부서에 근무하는 사원번호, 사원명, 부서명, 사원 조회하기. (비상관쿼리)
SELECT EMPLOYEE_ID  ,
       FIRST_NAME   , 
       LAST_NAME    , 
...
-- 부서번호가 50인 부서에 근무하는 사원번호, 사원명, 부서명, 사원 조회하기. (상관쿼리)
