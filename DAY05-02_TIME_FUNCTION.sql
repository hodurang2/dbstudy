...
SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD')
     ...
     , TO_CHAR(SYSTIMESTAMP, 'YYYY-MM-DD HH24:MI:SS.FF3') -- 밀리초(천분의1초) 포함 
  FROM DUAL;
...
--    1) INTERVAL 키워드 이용.
--    2) YEAR, MONTH, DAY, HOUR, MINUTE, SECOND 단위를 이용.
...
SELECT EXTRACT(YEAR FROM SYSDATE),                  -- 년
       ...
       EXTRACT(HOUR FROM SYSTIMESTAMP)+9,           -- 시 ASIA/DOKYO       
...
SELECT NEXT_DAY(SYSDATE, '수'),      -- 다음 수요일
       NEXT_DAY(SYSDATE-8, '수')     -- 이전 수요일(SYSDATE-7이 아님)
  FROM DUAL;
